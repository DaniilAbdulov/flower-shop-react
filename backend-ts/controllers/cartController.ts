...
class CartController {
    ...
    async createNewCartDataFromOrder(req: Request, res: Response) {
    ...
    async getCartData(req: Request, res: Response) {
        const userId: number = req.userId;
        if (!userId) {
            throw new Error("Некоректные данные");
        }
    ...
    async setCountOfItem(req: Request, res: Response) {
        const userId: number = req.userId;
        const productId: number = Number(req.body.params.id);
        const count: number = Number(req.body.params.newCount);
